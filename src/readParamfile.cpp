...
void readParamfile(string v_file, struct filekeywords *Paramfile){
  ...
  cout << "GULLS_STARS_DIR:" << Paramfile->starsdir << endl;

  if(tmp = getenv("GULLS_PLANETS_DIR"))
    {
      Paramfile->planetsdir = string(tmp); 
    }
  else
    {
      cout << "GULLS_PLANETS_DIR environment variable not set, assuming it is the same as GULLS_BASE_DIR" << endl;
      Paramfile->planetsdir = Paramfile->basedir;
    }
  cout << "GULLS_PLANETS_DIR:" << Paramfile->planetsdir << endl;
  ...
  cout << "GULLS_INPUT_DIR:" << Paramfile->inputdir << endl;
  ...
  Paramfile->planetdir = Paramfile->planetsdir + pfile["PLANET_DIR"];
